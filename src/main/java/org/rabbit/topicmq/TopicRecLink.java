...
//                System.out.println("cjson:" + cjson);
                JSONArray ja = JSONArray.fromObject(cjson);
//                System.out.println("jsonArray:"+ja.toString());
                if ( ja.size() > 0) {

                    for (int i = 0; i < ja.size(); i++) {
                        JsonModel jmSend = new JsonModel();
                        JSONObject cjs = ja.getJSONObject(i);//控制元
//                    System.out.println("json部位空,进入后台开始");
//                    JSONObject cjs = JSONObject.fromObject(cjson);
                        jmSend.setSensorId(cjs.getString("sei_id"));//控制
                        String sensor = sis.findSensorById(cjs.getString("sei_id"));
                        JSONObject sjs = JSONObject.fromObject(sensor);//控制传感器
                        //判断当前逻辑是否开启
...
//                            System.out.println("当前的状态不一致");
                                String tjson = tis.findTriggerBySid(sensorId,cjs.get("li_id").toString());
//                                System.out.println("tjson:"+tjson);
                                JSONObject tjs = JSONObject.fromObject(tjson);
                                boolean isOK = false;
                                //查询是否航油ｖａｌｕｅ值和ｖａｌｕｅ值是否部位空
//                                System.out.println("jm:"+JSONObject.fromObject(jm).toString());
...
//                                    System.out.println("条件达成发送控制");
                                        ...
                                        String fcode = sfis.selectFuncCode(jmSend.getSensorId(), sjs.getString("sti_id"), fuid);
                                        ...
                                        jmSend.setDeviceId(jm.getDeviceId());
                                        jmSend.setKey(jm.getKey());
                                        jmSend.setConnecttype(jm.getConnecttype());
                                        jmSend.setData(data);
//                                        String strSensor = sis.findSensorById(jmSend.getSensorId());   //获取控制传感器的类
//                                        JSONObject sjon = JSONObject.fromObject(strSensor);   //传感器的json包
                                        jmSend.setCode(sjs.getString("sei_mac"));   //控制器的code码
                                        jmSend.setTrantype(sjs.getString("tti_id")); //传输类型id
                                        if (jm.getConnecttype().equals("MQTT")) {//mqtt协议
//                                        System.out.println("MQTT发送给设备端:" + deviceId);
                                            mqttMessage.sendMsg("/downstream/" + deviceId, JSONObject.fromObject(jmSend).toString());
                                        }
                                    }
                                }
                            }
